...
- **🟢 Real Data Mode**: Live data from ESP32 weather sensors
- **🟠 Test Mode**: Generated test data for validation
...
│     - 6 Luzon stations with real-time updates        │
│     - Real/Test mode toggle button                   │
│     - Language toggle (EN/TL)                        │
│     - Station-specific sensors                       │
└────────────┬────────────────────────────────────────┘
             │ (React + TypeScript + Tailwind CSS)
             │
┌────────────▼────────────────────────────────────────┐
│    Node.js Backend (Express/Next.js API Routes)     │
│     - Serves web interface on port 3000              │
...
│                                                      │
│  ├── /weatherStations/        (Real data)           │
│  │   ├── manila, laguna, pampanga, cavite,          │
│  │   │   bulacan, batangas                          │
│  │   └── Data from ESP32 devices                    │
│  │                                                   │
│  └── /testWeatherStations/    (Test data)           │
│      ├── manila, laguna, pampanga, cavite,          │
│      │   bulacan, batangas                          │
│      └── Generated by Random toggle                 │
└──────────────────────────────────────────────────────┘

Optional: PyQt5 GUI (Raspberry Pi)
    └── Wraps the Next.js dashboard in a desktop window
```
...
| Cavite | Kawit | Temp, Humidity, Rain, Wind | — |
...
✅ **Installation complete!** Proceed to [Startup Methods](#startup-methods).
...
✅ **Raspberry Pi ready!** Proceed to [Startup Methods](#startup-methods).
...
📄 **Template File**: `ESP32_WEATHER_STATION_TEMPLATE.ino`
...
#### Option B: Automated Setup (Web Dashboard) ⭐ **NEW**
...
- ✅ Sensors pre-configured per station
- ✅ Faster deployment
- ✅ Fewer errors
- ✅ Ready-to-use code
...
- Updated by: Manual toggle (🎲 Random button)
...
1. **Status Bar**
   - Shows "✓ Server running on http://localhost:3000" (green)
   - Progress bar indicates loading status
   - Red warning if connection fails

2. **Control Buttons (Top)**
   - 🌐 **Open Dashboard**: Opens http://localhost:3000 in your default browser
   - ⚙️ **Settings**: Shows dashboard configuration and feature information
   - 🔄 **Refresh**: Reloads the embedded dashboard

3. **Dashboard Features (Inside)**
   - Real/Test mode toggle (🟢/🟠 buttons)
   - 6 Luzon stations with live data
   - Station-specific sensors displayed
   - 🎲 Random data generator (visible in Test mode)
   - Language toggle (EN/TL)
   - 🤖 ESP32 Customizer panel
...
```
🟢 Real Data button is active (green)
├── Displays 6 Luzon stations
├── Data from /weatherStations/ path
├── Updates from ESP32 sensors
└── Random toggle button: HIDDEN
```

**What you see:**
- Manila: 28.5°C with Pressure & Solar Radiation
- Laguna: 28.2°C with Soil Moisture
- Pampanga: 30.2°C with UV Index & Visibility
- Cavite: 29.1°C (main sensors only)
- Bulacan: 27.5°C with Soil Moisture & Pressure
- Batangas: 29.5°C with UV Index

#### 3. Switch to Test Mode

```
Click 🟠 "Test Mode" button
├── Displays 6 Luzon stations (same locations)
├── Data from /testWeatherStations/ path
├── Different values than real data
└── Random toggle button: NOW VISIBLE
```
...
```
Click 🎲 "Random" button
├── Toggles ON (blue highlight)
├── Updates test data every 5 seconds
├── Sends to /testWeatherStations/ path
└── Real data (/weatherStations/) UNCHANGED
```
...
├── Returns to /weatherStations/ path
├── Shows original ESP32 data
├── Random toggle button: HIDDEN
└── Data updates from sensors
```
...
└── Shows graph/chart (if enabled)
```

#### 7. Download Customized Arduino Code ⭐ **NEW**

For setting up new ESP32 weather stations directly from the dashboard:

```
Scroll to 🤖 "ESP32" Panel
│
├─ Click "Customize Sensors" button
│
├─ Dashboard expands with options:
│  ├─ Station Dropdown (Manila, Laguna, Pampanga, Cavite, Bulacan, Batangas)
│  └─ Sensor Checkboxes (Pressure, Solar, Soil, UV, Visibility)
│
├─ Select your station location
│
├─ Check sensors relevant to that station:
│  (Default: All sensors enabled)
│
└─ Click "Download Code"
   │
   └─ .ino file downloads with:
      ├─ Station location pre-filled
      ├─ Station name pre-filled
      ├─ Selected sensors already #defined
      └─ Ready to add WiFi & Firebase credentials
```
...
1. Ensure you're in 🟠 Test Mode (not Real Data)
...
2. **Verify correct mode:**
   ```
   🟢 Green = Real Data (/weatherStations/)
   🟠 Orange = Test Mode (/testWeatherStations/)
   ```
3. **Switch modes:**
